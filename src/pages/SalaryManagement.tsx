...
export default function SalaryManagement() {
  ...
  const addSalary = () => {
    ...
    const record: SalaryRecord = {
      id: Date.now().toString(), supervisorUsername: selectedSup, supervisorName: sup?.fullName || selectedSup,
      ...
      {canCredit && (
        <div className="data-card mb-4 space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
            ...
            <Button className="btn-press gap-1.5" onClick={addSalary}><IndianRupee className="h-4 w-4" /> Create</Button>
          </div>
          {selectedSup && (
          ...
          <tbody>
            {records.map((r) => (
              <tr key={r.id} className="border-b border-border last:border-0">
                <td className="py-2.5 pr-3 font-medium">{r.supervisorName}</td>
                ...
                <td className="py-2.5 pr-3">
                  <Badge variant={r.status === "credited" ? "default" : "secondary"}>{r.status === "credited" ? "Credited" : "Pending"}</Badge>
                </td>
                {canCredit && (
                  <td className="py-2.5">
                    {r.status === "pending" && (
                      <Button size="sm" className="btn-press" onClick={() => creditSalary(r.id)}>Credit Salary</Button>
                    )}
                  </td>
                )}
              </tr>
